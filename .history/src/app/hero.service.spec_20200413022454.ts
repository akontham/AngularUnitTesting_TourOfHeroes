  ...
  describe("getHero", () => {
    it("should call get with correct URL", () => {
      ...
      */
      ...
      httpTestingController.verify(); // verifies no unexpected requests are made.
    });

    it("should call get and verify the Hero returned", () => {
      heroService.getHero(3).subscribe((hero: Hero) => {
        expect(hero.id).toEqual(3);
        expect(hero.name).toEqual("Woderful Woman");
        expect(hero.strength).toEqual(100);
      });

      let req = httpTestingController.expectOne("api/heroes/3");
      req.flush({ id: 3, name: "Woderful Woman", strength: 100 });
